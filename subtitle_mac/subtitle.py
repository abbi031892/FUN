...
class Subtitle(object):
    ...
    def download_sub(self):
        ...
        for file_path in OpenSubs().download_sub(self.movie_path):
            ...
            with open(file_path) as f:
                for line in f:
                    if text_line:
                        time_text[time_key] = line
                        text_line = False
                        time_key = ''
                        continue
                    #pattern match on 'key : value'
                    field_match = re.match(regex_pattern, line)
                    if field_match:
                        time_text[line] = ''
                        text_line = True
                        time_key = line
            for time, text in time_text.iteritems():
                ...
                try:
                    ...
                    print("Wit.ai thinks you said " + data_text)
                except sr.UnknownValueError:
                    print("Wit.ai could not understand audio")
                except sr.RequestError as e:
                    print("Could not request results from Wit.ai service; {0}".format(e))
                print data_text, text
                if self.match(data_text, text):
                    print 'Fucking YES'
                    return
                else:
                    print 'SAD'
    ...
    def set_wav_path(self, start_time, end_time):
        ...
        with open(self.wav_path, 'w+'):
            to_wav_comm = "ffmpeg -y -i '{}' -map 0:1 -acodec pcm_s16le -ac 2 '{}'".format(self.mp4_path,
                                                                                           self.wav_path)
            print to_wav_comm
            subprocess.call(to_wav_comm, shell=True)

    def match(self, data_text, text):
        if not data_text or not text:
            return False
        return True if fuzz.ratio(data_text, text) > 50 else False
